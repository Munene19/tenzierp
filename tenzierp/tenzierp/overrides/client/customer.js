    ...
    if (!frm.is_new() && frm.doc.tax_id) {
      frm.add_custom_button(
        ...
        function () {
          frappe.call({
            method:
              "tenzierp.tenzierp.apis.apis.perform_customer_search",
            args: {
              request_data: {
                name: frm.doc.name,
      ...
      if (!frm.doc.custom_details_submitted_successfully) {
        frm.add_custom_button(
          ...
          function () {
            frappe.call({
              method:
                "tenzierp.tenzierp.apis.apis.send_branch_customer_details",
              args: {
                request_data: {
                  name: frm.doc.name,
    ...
    if (
      ...
      frm.add_custom_button(
        ...
        function () {
          frappe.call({
            method:
              "tenzierp.tenzierp.apis.apis.send_insurance_details",
            args: {
              request_data: {
                name: frm.doc.name,
                tax_id: frm.doc.tax_id,
